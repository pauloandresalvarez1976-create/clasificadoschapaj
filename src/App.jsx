...
function PublicarModal({ user, userData, onClose, onSuccess }) {
  ...
  const handleIA = async () => {
    ...
    try {
      const res = await fetch("/.netlify/functions/claude-ia", {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({
          model: "claude-haiku-4-5-20251001",
          max_tokens: 1000,
          messages:[{ role:"user", content:`Sos un asistente para un portal de clasificados en San Juan, Argentina llamado "Clasificados Chapa J".
El usuario escribió este anuncio: "${titulo}"
Generá en JSON (sin markdown):
{
  "titulo": "título optimizado de máx 70 chars",
  "descripcion": "descripción detallada de 3-4 oraciones naturales en español argentino",
  "precio_sugerido": "precio en ARS o 'Consultar'",
  "categoria": "una de estas: ${(window.__CATS__||DEFAULT_CATS).map(c=>c.name).join(", ")},"
  "estado": "nuevo o usado"
      ...
      if (json.precio_sugerido && json.precio_sugerido!=="Consultar") setPrecio(json.precio_sugerido.replace(/[^0-9]/g,""));
      if (json.categoria) setCategoria(json.categoria);
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.6)",backdropFilter:"blur(4px)",
      display:"flex",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto" }} onClick={onClose}>
      <div style={{ background:SF,borderRadius:20,padding:32,width:"100%",maxWidth:560,
        ...
        {step===1 && (
          <>
            <div style={{ background:`${P}11`,border:`1px solid ${P}33`,borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10 }}>
              <span style={{ fontSize:20 }}>🤖</span>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13,fontWeight:600,color:P }}>Asistente IA</div>
                <div style={{ fontSize:12,color:TM }}>Escribí qué vendés y la IA completa el formulario</div>
              </div>
              <Btn size="sm" onClick={handleIA} disabled={aiLoading}>{aiLoading?"Generando...":"Usar IA"}</Btn>
            </div>
...
function AnuncioDetalle({ anuncio, onClose, user }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.7)",backdropFilter:"blur(4px)",
      display:"flex",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto" }} onClick={onClose}>
      <div style={{ background:SF,borderRadius:20,width:"100%",maxWidth:760,maxHeight:"90vh",overflowY:"auto",
        boxShadow:"0 24px 80px rgba(0,0,0,.3)" }} onClick={e=>e.stopPropagation()}>

        {/* Fotos */}
        <div style={{ position:"relative",background:BG,borderRadius:"20px 20px 0 0",overflow:"hidden",height:300 }}>
          {fotos.length>0 ? (
            <>
          ...
          <button onClick={onClose} style={{ position:"absolute",top:12,right:12,background:"rgba(0,0,0,.5)",border:"none",color:"#fff",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontSize:18 }}>✕</button>
...
// ── NAVBAR ───────────────────────────────────────────────────────
function Navbar({ user, onLogin, onPublicar, onMiCuenta, onMensajes, unreadMsgs=0, searchQuery, setSearchQuery, onNavClick, siteInfo={} }) {
    ...
    <nav style={{ position:"sticky",top:0,zIndex:100,background:scrolled?"rgba(255,255,255,.97)":SF,
      ...
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"0 20px",display:"flex",alignItems:"center",gap:14,height:64 }}>
        ...
        <div style={{ flex:1,maxWidth:480,display:"flex" }}>
          ...
          <button onClick={()=>document.activeElement?.blur()} style={{ height:42,padding:"0 18px",background:P,color:"#fff",border:"none",
      ...
      <div style={{ borderTop:`1px solid ${BR}`,overflowX:"auto",scrollbarWidth:"none" }}>
        <div style={{ maxWidth:1200,margin:"0 auto",padding:"0 20px",display:"flex" }}>
          {["Vehículos","Inmuebles","Servicios","Artículos","Tiendas Virtuales","Últimos Publicados"].map(item=>(
            <button key={item} onClick={()=>onNavClick&&onNavClick(item)} style={{ padding:"9px 16px",border:"none",background:"transparent",color:TM,
              fontWeight:500,fontSize:13,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit",
              borderBottom:"2px solid transparent",transition:"all .2s" }}
              onMouseEnter={e=>{e.target.style.color=P;e.target.style.borderBottomColor=P;}}
              onMouseLeave={e=>{e.target.style.color=TM;e.target.style.borderBottomColor="transparent";}}
            >{item}</button>
          ))}
        </div>
      </div>
    ...
    try {
      const res = await fetch("/.netlify/functions/claude-ia", {
        ...
        body: JSON.stringify({
          ...
          messages:[{
            role:"user",
            content:`Sos un experto en marketing para comercios argentinos. Mejorá este anuncio para el sitio Clasificados Chapa J de San Juan, Argentina.
...
Respondé SOLO con JSON sin backticks, así:
{"titulo":"nuevo título atractivo","descripcion":"descripción mejorada de 2-3 oraciones llamativas","tip":"un consejo breve para vender más"}`
          }]
        })
      });
      const data = await res.json();
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
            <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
              ...
              <div style={{ background:BG,borderRadius:12,padding:"16px",border:`1px solid ${BR}` }}>
                ...
                <div style={{ marginBottom:10 }}>
                  <div style={{ fontSize:12,fontWeight:600,color:TL,marginBottom:4 }}>Descripción</div>
                  <textarea value={editForm.descripcion||""} onChange={e=>setEditForm(p=>({...p,descripcion:e.target.value}))} rows={3}
                    style={{ width:"100%",padding:"8px 12px",borderRadius:8,border:`1.5px solid ${BR}`,fontFamily:"inherit",fontSize:13,resize:"vertical",boxSizing:"border-box" }}/>
                </div>
                <div style={{ marginBottom:10 }}>
...
function PlanTab({ userData, user }) {
  ...
  const planes = [
    { id:"cuarzo",    icon:"🪨", label:"Cuarzo",    color:"#6B7280", bg:"#F9FAFB", border:"#D1D5DB", precio:"Gratis",   precioNum:0,                desc:"Publicá sin costo. Aparecés en el listado general.", features:["Hasta 5 fotos","Listado general","Sin vigencia"] },
    { id:"esmeralda", icon:"💚", label:"Esmeralda", color:"#16A34A", bg:"#F0FDF4", border:"#4ADE80", precio:`$${precioEsmeralda.toLocaleString("es-AR")} / mes`, precioNum:precioEsmeralda, precioDesc: descActivo?precioEsmDesc:null, desc:"Destacate en verde. Mayor visibilidad en búsquedas.", features:["Hasta 10 fotos","Badge verde destacado","Aparecés antes que Cuarzo",`${vigEsmeralda} días de vigencia`] },
    { id:"diamante",  icon:"💠", label:"Diamante",  color:"#7C3AED", bg:"#F5F3FF", border:"#8B5CF6", precio:`$${precioDiamante.toLocaleString("es-AR")} / mes`, precioNum:precioDiamante,  precioDesc: descActivo?precioDiaDesc:null, desc:"El plan premium. Primero en todo, máxima exposición.", features:["Hasta 20 fotos","Badge violeta premium","Primero en todos los listados",`${vigDiamante} días de vigencia`] },
  ];
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {planes.map(p=>(
          <div key={p.id} style={{ border:`2px solid ${planActual===p.id?p.color:p.border}`, borderRadius:14, padding:"18px 20px",
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                  ...
                  {planActual===p.id && <span style={{ fontSize:11, fontWeight:700, background:p.color, color:"#fff", padding:"2px 8px", borderRadius:20 }}>ACTIVO</span>}
                </div>
                <div style={{ fontSize:13, color:TM, marginBottom:8 }}>{p.desc}</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {p.features.map(f=>(
                    <span key={f} style={{ fontSize:11, color:p.color, background:p.bg, border:`1px solid ${p.border}`, padding:"2px 8px", borderRadius:20 }}>✓ {f}</span>
                  ))}
                </div>
              </div>
              <div style={{ textAlign:"center", flexShrink:0 }}>
                {p.id!=="cuarzo" && p.precioDesc ? (
                  <div style={{ marginBottom:8 }}>
                    <div style={{ fontSize:11, color:TL, textDecoration:"line-through", fontWeight:600 }}>{p.precio}</div>
                    <div style={{ fontWeight:900, fontSize:17, color:"#DC2626", lineHeight:1.1 }}>${p.precioDesc.toLocaleString("es-AR")}/mes</div>
                  ...
                  <div style={{ fontWeight:800, fontSize:15, color:p.id==="cuarzo"?OK:p.color, marginBottom:8 }}>{p.precio}</div>
                )}
                {p.id==="cuarzo" ? (
...
// ── MI CUENTA ────────────────────────────────────────────────────
function MiCuenta({ user, userData, onClose }) {
  ...
  return (
    ...
    <div style={{ position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.6)",backdropFilter:"blur(4px)",
      ...
      <div style={{ background:SF,borderRadius:20,width:"100%",maxWidth:700,maxHeight:"85vh",
        ...
        {/* Tabs */}
        <div style={{ display:"flex",borderBottom:`1px solid ${BR}`,padding:"0 24px" }}>
          {(["anuncios","perfil","plan",...(userData?.tiendaId?["tienda"]:[])]).map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{
              padding:"12px 16px",border:"none",background:"transparent",cursor:"pointer",
              fontFamily:"inherit",fontSize:13,fontWeight:700,
              color:tab===t?P:TM, borderBottom:`2px solid ${tab===t?P:"transparent"}`,
            }}>
              {t==="anuncios"?"📋 Mis Anuncios":t==="perfil"?"👤 Mi Perfil":t==="plan"?"💎 Mi Plan":"🏪 Mi Tienda"}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding:24 }}>
          {tab==="anuncios" && (
            loading ? <Spinner/> : misAnuncios.length===0 ? (
              <div style={{ textAlign:"center",padding:40,color:TL }}>
                <div style={{ fontSize:48,marginBottom:12 }}>📋</div>
                <div>No tenés anuncios publicados todavía</div>
              </div>
            ) : (
              misAnuncios.map(a=>{
                ...
                return (
                <div key={a.id} style={{ display:"flex",gap:12,padding:"14px 0",borderBottom:`1px solid ${BR}`,alignItems:"flex-start",
                  background: vencido ? "#FFF5F5" : "transparent", borderRadius: vencido ? 10 : 0,
                  padding: vencido ? "12px" : "12px 0" }}>
                  <div style={{ width:60,height:60,borderRadius:8,overflow:"hidden",flexShrink:0,background:BG,display:"flex",alignItems:"center",justifyContent:"center" }}>
                    {a.fotoPortada?<img src={a.fotoPortada} style={{ width:"100%",height:"100%",objectFit:"cover" }}/>:<span style={{ fontSize:24 }}>🖼️</span>}
                  </div>
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ fontWeight:700,fontSize:14,color:TX,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{a.titulo}</div>
                    <div style={{ fontSize:12,color:TL,marginBottom:2 }}>{a.categoria} · {a.localidad} · {timeAgo(a.createdAt)}</div>
                    <div style={{ fontSize:13,fontWeight:700,color:AC,marginBottom:4 }}>{a.precio==="Consultar"?"Consultar":`$${Number(a.precio).toLocaleString("es-AR")}`}</div>
                    {/* Vencimiento */}
                    {dias !== null && (
                      <div style={{ fontSize:11, fontWeight:700,
                        color: vencido ? ER : porVencer ? WA : TL,
                        background: vencido ? "#FEE2E2" : porVencer ? "#FFFBEB" : "transparent",
                        display:"inline-block", padding: vencido||porVencer ? "2px 8px" : 0,
                        borderRadius:20, marginBottom: vencido||porVencer ? 6 : 0 }}>
                        {vencido ? "⛔ Vencido" : porVencer ? `⚠️ Vence en ${dias} día${dias===1?"":"s"}` : `✅ Vence en ${dias} días`}
                      </div>
                    )}
                    {/* Botón renovar */}
                    {(vencido || porVencer || dias !== null) && (
                      esPago ? (
                        <div>
                          <button onClick={()=>setShowRenovarPago(a)}
                            style={{ display:"block", marginTop:4, padding:"5px 12px",borderRadius:8,
                              border:`1.5px solid ${P}`,color:"#fff",background:P,
                              cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit" }}>
                            🔄 Renovar — abonar nuevo período
                          </button>
                        </div>
                      ) : (
                        <button onClick={()=>handleRenovarGratis(a.id)} disabled={renovandoId===a.id}
                          style={{ display:"block", marginTop:4, padding:"5px 12px",borderRadius:8,
                            border:`1.5px solid ${OK}`,color:renovandoId===a.id?"#fff":OK,
                            background:renovandoId===a.id?OK:"transparent",
                            cursor:renovandoId===a.id?"not-allowed":"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit" }}>
                          {renovandoId===a.id ? "Renovando..." : "🔄 Renovar gratis — 30 días más"}
                        </button>
                      )
                    )}
                  </div>
                  <div style={{ display:"flex",flexDirection:"column",gap:6,flexShrink:0 }}>
                    <span style={{ padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:700,
                      background: vencido ? ER+"22" : a.status==="activo"?OK+"22":ER+"22",
                      color: vencido ? ER : a.status==="activo"?OK:ER }}>
                      {vencido ? "vencido" : a.status}
                    </span>
                    <div style={{ display:"flex",gap:4 }}>
                      {a.status==="activo" && !vencido
                        ? <button onClick={()=>handleSuspender(a.id)} style={{ padding:"4px 8px",borderRadius:6,border:`1px solid ${WA}`,color:WA,background:"transparent",cursor:"pointer",fontSize:11,fontFamily:"inherit" }}>Pausar</button>
                        : !vencido && <button onClick={()=>handleActivar(a.id)} style={{ padding:"4px 8px",borderRadius:6,border:`1px solid ${OK}`,color:OK,background:"transparent",cursor:"pointer",fontSize:11,fontFamily:"inherit" }}>Activar</button>
                      }
                      <button onClick={()=>handleEliminar(a.id)} style={{ padding:"4px 8px",borderRadius:6,border:`1px solid ${ER}`,color:ER,background:"transparent",cursor:"pointer",fontSize:11,fontFamily:"inherit" }}>Borrar</button>
                    </div>
                  </div>
                </div>
                );
          ...
          {tab==="plan" && (
            <PlanTab userData={userData} user={user} />
          )}
          {tab==="tienda" && userData?.tiendaId && (
            <MiTiendaTab user={user} userData={userData} tiendaId={userData.tiendaId}/>
          )}
...
function TiendaDetalle({ tienda, onClose, onVerAnuncio, siteWhatsapp }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:250,background:"rgba(0,0,0,.7)",backdropFilter:"blur(4px)",
      overflowY:"auto" }} onClick={onClose}>
...
function FrontSite({ user, userData, onLogin, onPublicar, onMiCuenta, onLegal }) {
      ...
      if(snap.exists()){
        if(snap.data().design) {
          ...
          if(snap.data().design.faviconUrl){
            // Buscar cualquier link de favicon existente y actualizarlo
            ...
            link.type = "image/png";
            link.href = snap.data().design.faviconUrl + "?t=" + Date.now(); // forzar recarga
          }
        }
  ...
  console.log("STATE search:", search, "recentAds:", recentAds.length);
  const filteredAds = recentAds.filter(a=>{
    if(search) return a.titulo?.toLowerCase().includes(search.toLowerCase()) || a.categoria?.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div style={{ fontFamily:"'Nunito','Segoe UI',sans-serif",background:BG,minHeight:"100vh",overflowX:"hidden" }}>
      <Navbar user={user} onLogin={onLogin} onPublicar={onPublicar} onMiCuenta={onMiCuenta}
        ...
        siteInfo={siteInfo}
        searchQuery={search} setSearchQuery={v=>{ setSearch(v); if(v){setSelCat(null);setSelSub(null);} }}
        onNavClick={(item)=>{
          if(item==="Últimos Publicados"){setSelCat(null);setSelSub(null);setTimeout(()=>anunciosRef.current?.scrollIntoView({behavior:"smooth",block:"start"}),100);}
      ...
      <section style={{ maxWidth:1200,margin:"0 auto",padding:"44px 20px 0" }}>
        <h2 style={{ fontFamily:"'Georgia',serif",fontSize:24,fontWeight:700,color:TX,margin:"0 0 20px" }}>Explorá por categoría</h2>
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:10 }}>
          {cats.map(cat=>{
            const [h,setH]=useState(false);
            return (
              <div key={cat.id} onClick={()=>handleSelCat(cat)}
                onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
                style={{ background:h||selCat?.id===cat.id?cat.color:SF,border:`1.5px solid ${h||selCat?.id===cat.id?cat.color:BR}`,
                  borderRadius:14,padding:"16px 12px",cursor:"pointer",textAlign:"center",transition:"all .25s",
                  transform:h?"translateY(-3px)":"none" }}>
                <div style={{ fontSize:26,marginBottom:4 }}>{cat.icon}</div>
                <div style={{ fontWeight:700,fontSize:12,color:h||selCat?.id===cat.id?"#fff":TX,transition:"color .25s" }}>{cat.name}</div>
              </div>
            );
          })}
        </div>
        {selCat && (
          <div style={{ marginTop:12,padding:14,background:SF,borderRadius:12,border:`1.5px solid ${selCat.color}22`,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center" }}>
            <span style={{ fontSize:12,color:TL,fontWeight:600,marginRight:4 }}>Subcategoría:</span>
            {(selCat.sub||[]).map(s=>(
              <button key={s} onClick={()=>handleSelSub(s)}
                style={{ padding:"5px 13px",borderRadius:20,border:`1px solid ${selCat.color}`,
                  background:selSub===s?selCat.color:"transparent",
                  color:selSub===s?"#fff":selCat.color,
                  cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:600,transition:"all .2s" }}>{s}</button>
            ))}
            <button onClick={()=>{setSelCat(null);setSelSub(null);}} style={{ marginLeft:"auto",background:"none",border:"none",color:TL,cursor:"pointer",fontSize:18 }}>✕</button>
          </div>
        )}
      </section>
      ...
      <section ref={anunciosRef} style={{ maxWidth:1200,margin:"0 auto",padding:"0 20px" }}>
        <div style={{ display:"flex",alignItems:"center",gap:12,margin:"0 0 20px",flexWrap:"wrap" }}>
          <h2 style={{ fontFamily:"'Georgia',serif",fontSize:24,fontWeight:700,color:TX,margin:0 }}>
            {selCat ? `${selCat.icon} ${selCat.name}${selSub?` · ${selSub}`:""}` : "🕐 Últimos Publicados"}
          </h2>
          {selCat && (
          ...
          {filteredAds.length>0 && <span style={{ fontSize:13,color:TL,marginLeft:"auto" }}>{filteredAds.length} anuncio{filteredAds.length!==1?"s":""}</span>}
        </div>
        {loadingAds ? <Spinner/> : filteredAds.length===0 ? (
          <div style={{ textAlign:"center",padding:40,color:TL }}>
            <div style={{ fontSize:48,marginBottom:12 }}>🔍</div>
            <div>{selCat?`Todavía no hay anuncios en ${selCat.name}. ¡Sé el primero en publicar!`:search?"No se encontraron anuncios":"Todavía no hay anuncios. ¡Sé el primero en publicar!"}</div>
            {selCat && <button onClick={()=>{setSelCat(null);setSelSub(null);}} style={{ marginTop:14,padding:"8px 20px",borderRadius:20,background:P,color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontFamily:"inherit" }}>Ver todos los anuncios</button>}
          </div>
        ) : (
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(175px,1fr))",gap:12 }}>
            {filteredAds.map(ad=><AdCard key={ad.id} ad={ad} onClick={setSelAnuncio}/>)}
          </div>
        )}
      </section>
                  ...
                  <div key={l} style={{ marginBottom:7 }}>
                    <a href="#" onClick={e=>{
                      ...
                      if(col.title==="Categorías"){
                        ...
                        if(l==="Cómo publicar") onPublicar();
                        else if(l==="Planes Cuarzo, Esmeralda y Diamante") onMiCuenta();
                        else if(l==="Contacto") {
                          const wa = "549" + siteWhatsapp;
                          const msg = encodeURIComponent("Hola! Tengo una consulta sobre Clasificados Chapa J.");
                          window.open(`https://wa.me/${wa}?text=${msg}`, "_blank");
                        }
                        else if(l==="Seguridad") onLegal();
                      }
...
const INIT_CFG = {
  ...
  pricing: {
    normalFree:true,
    plataPrice:"4600", plataPriceArticulos:"2300", plataVigencia:"30",
...
function AAds() {
  ...
  return (
    <div>
      ...
      {planModal && (
        <div style={{ position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,.6)",backdropFilter:"blur(4px)",
          display:"flex",alignItems:"center",justifyContent:"center",padding:20 }} onClick={()=>setPlanModal(null)}>
...
function AStores() {
  ...
  const [loading, setLoading]   = useState(true);
  const [editando, setEditando] = useState(null); // null=lista, "new"=nuevo, id=editar
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18 }}>
        <Card>
          ...
          <Inp label="💲 Destacado ESMERALDA (otros)" value={cfg.plataPriceArticulos} onChange={set("plataPriceArticulos")} type="number" />
...
function LegalView({ onVolver }) {
  const [pagina, setPagina] = useState("terminos");
  return (
    <div style={{ fontFamily:"Nunito,sans-serif", background:BG, minHeight:"100vh" }}>
      ...
      <div style={{ background:SF, borderBottom:`1px solid ${BR}`, padding:"0 20px", display:"flex" }}>
        {[{ id:"terminos", label:"📄 Términos y Condiciones" },{ id:"privacidad", label:"🔒 Política de Privacidad" }].map(t=>(
          <button key={t.id} onClick={()=>setPagina(t.id)} style={{ padding:"14px 20px", border:"none", background:"transparent", fontFamily:"inherit", fontSize:13, fontWeight:700, cursor:"pointer", color:pagina===t.id?P:TM, borderBottom:`3px solid ${pagina===t.id?P:"transparent"}` }}>{t.label}</button>
        ))}
      </div>
      <div style={{ maxWidth:860, margin:"0 auto", padding:"40px 20px 60px" }}>
        {pagina==="terminos" ? <Terminos/> : <Privacidad/>}
      </div>
...
export default function App() {
  ...
  return (
    <>
      ...
      {view==="legal" && (
        <LegalView onVolver={()=>setView("front")}/>
      )}
      ...
      {showMiCuenta && user && (
        <MiCuenta user={user} userData={userData} onClose={()=>setShowMiCuenta(false)}/>
      )}
